...
def Get_Net_res(oriImg, multiplier):
...
def Get_peaks(heatmap_avg):
    ...
    return all_peaks, all_peaks_list

def Get_PAF(paf_avg, all_peaks, oriImg):
    connection_all = []
    special_k = []
    mid_num = 10
...
def Get_human_subset(all_peaks, special_k, connection_all):
...
def Get_human_grid(subset, all_peaks_list):
    human_grid = []
    for human_i in subset:
        ...
        for i in range(18):
            ...
            if body_part_index == -1:
                ...
                grid = all_peaks_list[body_part_index]
...
def Get_Feature(img_dir):
    oriImg = cv.imread(img_dir)
    multiplier = [x * model['boxsize'] / oriImg.shape[0] for x in param['scale_search']]
    heatmap_avg, paf_avg = Get_Net_res(oriImg, multiplier)
    all_peaks, all_peaks_list = Get_peaks(heatmap_avg)
    special_k, connection_all = Get_PAF(paf_avg, all_peaks, oriImg)
    subset_ori = Get_human_subset(all_peaks, special_k, connection_all)
    subset = subset_filter(subset_ori)
    all_human_grid = Get_human_grid(subset, all_peaks_list)
    return all_human_grid
...
def main():
    ...
    for image_i in img_list:
        ...
        try:
            single_res = Get_Feature(img_dir)
            feature_res[img_dir] = single_res
            end_t = time.time()
    ...
    pickle.dump(feature_res, output)
    output.close()

    ff_res = pickle.load(open(save_path,"r"))
    print ff_res
    for i in ff_res:
        print "=========================================="
        print i
